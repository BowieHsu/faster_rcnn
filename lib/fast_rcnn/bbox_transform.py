...
#         if targets_theta[i] > (np.pi * 0.75):
#             while(targets_theta[i] > (np.pi * 0.75)):
#                 targets_theta[i] -= np.pi
#         elif targets_theta[i] < (-np.pi * 0.25):
#             while(targets_theta[i] < - (np.pi * 0.25)):
#                 targets_theta[i] += np.pi
#     #targets_theta = 0
#
#     targets = np.vstack(
...
def bbox_transform_inv(boxes, deltas):
    ...
    for i in range(len(pred_theta)):
        if pred_theta[i][0] > np.pi:
            while(pred_theta[i][0] > np.pi):
                pred_theta[i][0] -= 2 * np.pi
        elif pred_theta[i][0] < -np.pi :
            while(pred_theta[i][0] < - np.pi):
                pred_theta[i][0] += 2 * np.pi
